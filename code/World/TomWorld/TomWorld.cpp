...
shared_ptr<ParameterLink<int>> TomWorld::evaluationsPerGenerationPL =
    Parameters::register_parameter("WORLD_Tom-evaluationsPerGeneration", 30,
...
TomWorld::TomWorld(shared_ptr<ParametersTable> PT) : AbstractWorld(PT) {
...
auto TomWorld::evaluate(map<string, shared_ptr<Group>>& groups, int analyze, int visualize, int debug) -> void {
...
auto TomWorld::requiredGroups() -> unordered_map<string,unordered_set<string>> {
